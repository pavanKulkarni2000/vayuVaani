...
import com.example.vayuVaani.util.fileType
...
class FolderMediaListFragment(private val currentFolder:File) :Fragment(R.layout.list_layout){
        ...
        scope.launch {
            val newFiles =
                if (currentFolder.isUri) {
                    ...
                    getMediaInFolder(Paths.get(currentFolder.path)).map {
                        File.fromPath(
                            it,
    ...
    private fun mediaOnClick(file: File) {
        parentFragmentManager.commit {
            PlayerFragment(file).let {
                add(R.id.fragment_container_view, it, TAG)
            }
            addToBackStack(null)
        }
    }
